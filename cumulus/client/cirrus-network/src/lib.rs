...
const LOG_TARGET: &str = "sync::cirrus";
...
impl Decode for BlockAnnounceData {
	fn decode<I: codec::Input>(input: &mut I) -> Result<Self, codec::Error> {
		...
		let relay_parent = match PHash::decode(input) {
			...
			Err(_) => panic!("Can not decode PHash"),
		};

		Ok(Self { receipt, relay_parent })
	}
}
...
impl TryFrom<&'_ CollationSecondedSignal> for BlockAnnounceData {
	type Error = ();
	fn try_from(signal: &CollationSecondedSignal) -> Result<Self, ()> {
		Ok(Self {
			receipt: b"Receipt converted from CollationSecondedSignal".to_vec(),
	...
	B: Backend<PBlock> + 'static,
{
	...
	fn handle_empty_block_announce_data(
		...
		ready(Ok(Validation::Success { is_new_best: false }))
		/*
		async move {
	...
	BCE: BlockchainEvents<PBlock> + 'static + Send + Sync,
{
	fn validate(
		...
		ready(Ok(Validation::Success { is_new_best: false })).boxed()
...
pub fn build_block_announce_validator<Block: BlockT, B>(
	relay_chain_client: Arc<subspace_node::service::FullClient>,
	// para_id: ParaId,
	relay_chain_sync_oracle: Box<dyn SyncOracle + Send>,
	relay_chain_backend: Arc<B>,
