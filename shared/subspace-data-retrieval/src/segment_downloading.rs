...
///
/// Prefers source pieces if available, on error returns the number of available pieces.
pub async fn download_segment_pieces_once<PG>(
    segment_index: SegmentIndex,
    piece_getter: &PG,
) -> Result<Vec<Option<Piece>>, SegmentDownloadingError>
where
    PG: PieceGetter,
{
    match download_missing_segment_pieces(segment_index, piece_getter, NO_EXISTING_PIECES).await {
        Ok(segment_pieces) => Ok(segment_pieces),
        Err((error, _incomplete_segment_pieces)) => Err(error),
    }
}
