...
pub enum Error {
    ...
    #[error(
        "Object length extends beyond block continuation: {segment_data_length:?}, \
         next source piece: {next_source_piece_index:?}, remaining_piece_count: \
        {remaining_piece_count}, object: {mapping:?}"
    )]
    ObjectOutsideBlock {
        ...
        /// The segment data lengths, after the concurrent downloads
        ...
        let mut partial_object = loop {
                ...
                PartialObject::new_with_padding(&partial_data, self.max_object_len, mapping)?
            {
                ...
                trace!(
                    ...
                    ?mapping,
                    ?partial_object,
                    "Successfully decoded partial object length from first piece(s)",
                );
