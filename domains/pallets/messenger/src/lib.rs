...
mod pallet {
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub fn relay_message(
            origin: OriginFor<T>,
            msg: CrossDomainMessage<T::Hash, T::MmrHash>,
        ) -> DispatchResult {
            ensure_none(origin)?;
    ...
    impl<T: Config> Pallet<T> {
        ...
        pub(crate) fn do_verify_xdm(
            ...
            let mmr_leaf = EncodableOpaqueLeaf(
                xdm.proof
            ...
            let mmr_proof = xdm.proof.consensus_chain_mmr_proof.proof.clone();
            ...
            let state_root = if let Some(domain_proof) = xdm.proof.domain_proof.clone()
                && let Some(domain_id) = xdm.src_chain_id.is_domain_chain()
            {
