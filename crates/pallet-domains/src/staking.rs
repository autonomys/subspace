...
pub enum OperatorStatus<DomainBlockNumber, ReceiptHash> {
    ...
    /// Operator was deactivated due to being offline.
    /// Operator can be activated after given epoch_index.
    #[codec(index = 5)]
    Deactivated(EpochIndex),
}
    ...
    Operator<Balance, Share, DomainBlockNumber, ReceiptHash>
{
    ...
    /// Returns true if the operator is either Registered or Deactivated.
    pub fn is_operator_registered_or_deactivated<T: Config>(
        &self,
        operator_id: OperatorId,
    ) -> bool {
...
pub(crate) fn do_withdraw_stake<T: Config>(
    ...
    Operators::<T>::try_mutate(operator_id, |maybe_operator| {
        ...
        // if operator is deactivated, to calculate the share price at the end of the epoch,
        // add them to DeactivatedOperators Storage
        if matches!(operator.partial_status, OperatorStatus::Deactivated(_)) {
            DeactivatedOperators::<T>::mutate(operator.current_domain_id, |operators| {
                operators.insert(operator_id)
            });
...
pub(crate) mod tests {
    ...
    use crate::staking::{
        DomainEpoch, Error as StakingError, Operator, OperatorConfig, OperatorDeregisteredInfo,
        OperatorStatus, SharePrice, StakingSummary, do_convert_previous_epoch_withdrawal,
    ...
    fn operator_deactivation() {
        ...
        ext.execute_with(|| {
            ...
            let domain_stake_summary = DomainStakingSummary::<Test>::get(domain_id).unwrap();
            let total_current_stake = domain_stake_summary.current_total_stake;
            ...
            assert_eq!(
                total_current_stake,
                domain_stake_summary.current_total_stake + operator_stake
            );

            // operator nomination will not work since the operator is deactivated
            let nominator_account = 100;
            let nominator_stake = 100 * AI3;
            let res = Domains::nominate_operator(
                RuntimeOrigin::signed(nominator_account),
    ...
    #[test]
    fn operator_deactivation_withdraw_stake() {
        let domain_id = DomainId::new(0);
        let operator_account = 1;
        let operator_stake = 200 * AI3;
        let operator_free_balance = 250 * AI3;
        ...
        ext.execute_with(|| {
            ...
            // operator should be part of the DeactivatedOperator storage
            assert!(DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));

            // transition epoch
            do_finalize_domain_current_epoch::<Test>(domain_id).unwrap();

            // operator not should be part of the DeactivatedOperator storage after epoch transition
            assert!(!DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));

            // nominator withdraw should work even though operator is deactivated
            let nominator_shares = Perquintill::from_percent(80).mul_floor(nominator_total_stake);
            ...
            // operator should be part of the DeactivatedOperator storage since there is a new
            // withdrawal and share prices needs to be calculated.
            assert!(DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));
            ...
            // share price should exist for previous epoch
            let domain_epoch: DomainEpoch = (domain_id, current_epoch_index).into();
            assert!(OperatorEpochSharePrice::<Test>::get(operator_id, domain_epoch).is_some());

            let nominator_withdrawals =
                Withdrawals::<Test>::get(operator_id, nominator_account).unwrap();
            let unlock_funds_at = nominator_withdrawals
                .withdrawal_in_shares
                .unwrap()
                .unlock_at_confirmed_domain_block_number;
            HeadDomainNumber::<Test>::insert(domain_id, unlock_funds_at + 1);

            // unlocking nominator funds should work when operator is still deactivated
            let res = Domains::unlock_funds(RuntimeOrigin::signed(nominator_account), operator_id);
            assert_ok!(res);
        });
    }

    #[test]
    fn operator_deactivation_deregister_operator() {
        let domain_id = DomainId::new(0);
        let operator_account = 1;
        let operator_stake = 200 * AI3;
        let operator_free_balance = 250 * AI3;
        ...
        ext.execute_with(|| {
            ...
            // operator deregistration should work even if operator is deactivated
            let res =
                Domains::deregister_operator(RuntimeOrigin::signed(operator_account), operator_id);
            assert_ok!(res);

            // operator not should be part of the DeactivatedOperator storage since operator is
            // deregistered but instead should be part of DeregisteredOperators storage
            assert!(!DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));
            assert!(DeregisteredOperators::<Test>::get(domain_id).contains(&operator_id));
            ...
            let unlock_operator_at = HeadDomainNumber::<Test>::get(domain_id)
                + <Test as Config>::StakeWithdrawalLockingPeriod::get();

            let operator = Operators::<Test>::get(operator_id).unwrap();
            assert_eq!(
                operator.partial_status,
                OperatorStatus::Deregistered(OperatorDeregisteredInfo {
                    domain_epoch,
                    unlock_at_confirmed_domain_block_number: unlock_operator_at
                })
            );

            HeadDomainNumber::<Test>::insert(domain_id, unlock_operator_at);

            // unlocking nominator should work
            let res =
                Domains::unlock_nominator(RuntimeOrigin::signed(nominator_account), operator_id);
            assert_ok!(res);

            // unlocking operator also should work
            let res =
                Domains::unlock_nominator(RuntimeOrigin::signed(operator_account), operator_id);
            assert_ok!(res);

            // cleanup of operator should be done
    ...
    fn operator_reactivation() {
        ...
        ext.execute_with(|| {
            ...
            // reactivation should not work before cool off period
            ...
            // operator not should be part of the DeactivatedOperator storage
            assert!(!DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));
            ...
            // operator should be part of the DeactivatedOperator storage since there is a withdrawal
            assert!(DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));
            ...
            // operator not should be part of the DeactivatedOperator storage since operator is
            // reactivated and moved to next operator set
            assert!(!DeactivatedOperators::<Test>::get(domain_id).contains(&operator_id));
    ...
    fn withdraw_stake(params: WithdrawParams) {
        withdraw_stake_inner(params, false).expect("always panics rather than returning an error");
    }

    /// Withdraw stake in a proptest.
    fn withdraw_stake_prop(params: WithdrawParams) -> TestCaseResult {
        withdraw_stake_inner(params, true)
    }

    /// Inner function for withdrawing stake.
    fn withdraw_stake_inner(params: WithdrawParams, is_proptest: bool) -> TestCaseResult {
        let WithdrawParams {
            minimum_nominator_stake,
            nominators,
            operator_reward,
        ...
        ext.execute_with(|| {
            ...
            for (withdraw, expected_result) in withdraws {
                ...
                if is_proptest {
                    prop_assert_eq!(
                        ...
                        "unexpected withdraw_stake result",
                    );
                } else {
                    assert_eq!(
                        res,
            ...
            if let Some((withdraw, include_ed)) = expected_withdraw {
                ...
                if is_proptest {
                    ...
                    assert_eq!(
                        ...
                        expected_balance,
                        "usable balance is not equal to expected balance:\n{} !=\n{}",
                        Balances::usable_balance(nominator_id),
                        expected_balance,
                    );
                }
            ...
            if expected_nominator_count_reduced_by > 0 {
                if is_proptest {
                    prop_assert!(
                        Deposits::<Test>::get(operator_id, nominator_id).is_none(),
                        "deposit exists for nominator: {nominator_id}",
                    );
                    prop_assert!(
                        !DepositOnHold::<Test>::contains_key((operator_id, nominator_id)),
                        "deposit on hold exists for nominator: {nominator_id}",
                    );
                } else {
                    assert!(
                        Deposits::<Test>::get(operator_id, nominator_id).is_none(),
            ...
            if is_proptest {
                ...
                assert_eq!(
                    Balances::usable_balance(nominator_id)
                        ...
                        + bundle_storage_fund::total_balance::<Test>(operator_id),
                    total_balance,
                    "initial total balance is not equal to final total balance:\n\
                    {} + {} + {} =\n{} !=\n{}",
                    Balances::usable_balance(nominator_id),
                    operator.current_total_stake,
                    bundle_storage_fund::total_balance::<Test>(operator_id),
    ...
    ///
    /// Limiting the balances avoids arithmetic errors in the withdraw_stake test function, and
    ...
    /// Property test for withdrawing an operator's excess stake.
    ...
    fn prop_withdraw_excess_operator_stake() {
        prop_test!(&PROP_OPERATOR_STAKE_RANGE, |operator_stake| {
            ...
            excess_stake = Perquintill::from_parts(1)
                .left_from_one()
                .mul_ceil(excess_stake);
            excess_stake -= 1;

            prop_assert!(excess_stake > 0, "would cause ZeroWithdraw error");

            let expected_withdraw = (excess_stake, false);

            withdraw_stake_prop(WithdrawParams {
                minimum_nominator_stake: <Test as Config>::MinOperatorStake::get(),
    ...
    fn prop_withdraw_excess_nominator_stake_with_deposit() {
        prop_test!(
            ...
            |(operator_stake, mut nominator_stake, maybe_deposit)| {
                ...
                // Some deposit gets left as storage fees.
                let expected_withdraw = (
                    STORAGE_FEE_RESERVE
                        .left_from_one()
                        .mul_ceil(nominator_stake)
    ...
    /// Property test for withdrawing an operator's excess stake with a nominator and a reward.
    /// The total balance should be increased by part of the reward afterwards.
    #[test]
    fn prop_withdraw_excess_operator_stake_with_nominator_and_reward() {
        prop_test!(
            ...
            |(operator_stake, nominator_stake, operator_reward)| {
                ...
                // Withdraw is stake plus reward split.
                let total_stake = operator_stake.saturating_add(nominator_stake);
                let operator_reward_split = Perquintill::from_rational(operator_stake, total_stake)
                    .mul_floor(operator_reward);

                // Shares are approximately equal to the original stake.
                // TODO: fix the tests so we can go as low as MinOperatorStake
                let excess_stake =
                    operator_stake.saturating_sub(2 * <Test as Config>::MinOperatorStake::get());

                let expected_withdraw = (excess_stake.saturating_add(operator_reward_split), true);
    ...
    /// Property test for withdrawing an operator's excess stake with a deposit and a reward.
    /// Their balance should be almost the same before and after.
    #[test]
    fn prop_withdraw_excess_operator_stake_with_deposit_and_reward() {
        prop_test!(
            ...
            |(mut operator_stake, maybe_deposit, operator_reward)| {
                ...
                // Total balances can't overflow: arithmetic overflow error in withdraw_stake test function
                prop_assert!(
                    [operator_stake, maybe_deposit, operator_reward]
                        .into_iter()
                        .try_fold(0_u128, |acc, value| acc.checked_add(value))
                        .is_some()
                );
                ...
                // Shares are approximately equal to the original stake.
                // TODO: fix the tests so we can go as low as MinOperatorStake
                let reserve = 2 * <Test as Config>::MinOperatorStake::get();
                let excess_stake = operator_stake.saturating_sub(reserve);

                // Withdraw is stake plus reward, but some deposit gets left as storage fees.
                let expected_withdraw = (
                    operator_stake
