...
pub(crate) const TEST_GRANDPA_ROUND: u64 = 1;
pub(crate) const TEST_GRANDPA_SET_ID: SetId = 1;

/// Configuration parameters when generating test GRANDPA justifications.
#[derive(Clone)]
pub(crate) struct JustificationGeneratorParams<H> {
...
pub(crate) fn make_default_justification<H: HeaderT>(header: &H) -> GrandpaJustification<H> {
...
pub(crate) fn make_justification_for_header<H: HeaderT>(
...
pub(crate) fn signed_precommit<H: HeaderT>(
    signer: &Account,
...
pub(crate) fn test_header<H: HeaderT>(number: H::Number) -> H {
    let default = |num| {
        H::new(
            num,
...
pub(crate) fn header_id<H: HeaderT>(index: u8) -> (H::Hash, H::Number) {
