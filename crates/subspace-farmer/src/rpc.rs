...
pub trait RpcClient {
    /// Get farmer metadata
    async fn farmer_metadata(&mut self) -> Result<FarmerMetadata, Error>;
    ...
    async fn subscribe_new_head(&mut self) -> Result<Receiver<NewHead>, Error>;

    /// Subscribe to slot
    async fn subscribe_slot_info(&mut self) -> Result<Receiver<SlotInfo>, Error>;
