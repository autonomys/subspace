...
use tokio::task;
use tracing::{Instrument, Span, error, info, trace, warn};
...
enum PlotMetadataVersion {
    /// Original layout, before the abundance-backed proof of space.
    #[codec(index = 0)]
    V0,
    /// Adds the `cutover`, past which sectors use the abundance-backed proof of space.
    #[codec(index = 1)]
    V1,
}
        ...
        for sector_index in 0..metadata_header.plotted_sector_count {
            ...
            sectors_metadata.push(
                SectorMetadataChecksummed::decode(&mut sector_metadata_bytes.as_ref()).map_err(
                    |error| io::Error::other(format!("Failed to decode sector metadata: {error}")),
                )?,
            );
        }
        ...
        if target.metadata() || target.plot() {
            ...
            let (metadata_file, mut metadata_header) = {
                ...
                if metadata_size < expected_metadata_size {
                    ...
                    if !dry_run
                        && let Err(error) = metadata_file.write_all_at(&metadata_header_bytes, 0)
                    {
                        ...
                        if target.plot() {
                            ...
                            if actual_checksum != expected_checksum {
                                    ...
                                    sector_bytes_range.clone().step_by(scratch_buffer.len())
                                {
                                    ...
                                    if !dry_run
                                        && let Err(error) = plot_file.write_all_at(bytes, offset)
                                    {
                                        return Err(SingleDiskFarmScrubError::FailedToWriteBytes {
        ...
        (0..number_of_cached_elements)
            ...
            .map_with(vec![0; element_size as usize], |element, cache_offset| {
                ...
                if let Err(error) = cache_file.read_exact_at(element, offset) {
                    ...
                    if !dry_run && let Err(error) = cache_file.write_all_at(&dummy_element, offset)
                    {
                        return Err(SingleDiskFarmScrubError::FailedToWriteBytes {
                            file: file.clone(),
