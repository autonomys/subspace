...
You need 2 executables (1 for farmer, 1 for node). Download the executables `subspace-node-...` and `subspace-farmer-...` for your operating system.
...
4. This will start the node (replace `INSERT_YOUR_ID` with a nickname you choose): `./subspace-node-x86_64-*-snapshot --chain testnet --name INSERT_YOUR_ID --rpc-cors all --ws-external --rpc-methods unsafe --wasm-execution compiled --execution wasm --validator --bootnodes "/dns/farm-rpc.subspace.network/tcp/30333/p2p/12D3KooWPjMZuSYj35ehced2MTJFf95upwpHKgKUrFRfHwohzJXr" --telemetry-url "wss://telemetry.polkadot.io/submit/ 1"` 
...
3. This will start the node (replace `INSERT_YOUR_ID` with a nickname you choose): `subspace-node-x86_64-*-snapshot --chain testnet --name INSERT_YOUR_ID --rpc-cors all --ws-external --rpc-methods unsafe --wasm-execution compiled --execution wasm --validator --bootnodes "/dns/farm-rpc.subspace.network/tcp/30333/p2p/12D3KooWPjMZuSYj35ehced2MTJFf95upwpHKgKUrFRfHwohzJXr" --telemetry-url "wss://telemetry.polkadot.io/submit/ 1"`
