...
RUN \
    ...
    /root/.cargo/bin/cargo -Zgitoxide -Zgit build \
        ...
        -Z build-std \
        --profile $PROFILE \
        --package evm-domain-runtime \
        --target $RUSTC_TARGET_ARCH-unknown-linux-gnu && \
    mv \
      target/*/*/wbuild/evm-domain-runtime/evm_domain_runtime.compact.compressed.wasm \
      evm_domain_runtime.compact.compressed.wasm && \
    rm -rf target

ENTRYPOINT ["/usr/bin/cat", "evm_domain_runtime.compact.compressed.wasm"]
