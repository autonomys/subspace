...
RUN \
    ...
    /root/.cargo/bin/cargo -Zgitoxide -Zgit build \
        ...
        -Z build-std \
        --profile $PROFILE \
        --package auto-id-domain-runtime \
        --target $RUSTC_TARGET_ARCH-unknown-linux-gnu && \
    mv \
      target/*/*/wbuild/auto-id-domain-runtime/auto_id_domain_runtime.compact.compressed.wasm \
      auto_id_domain_runtime.compact.compressed.wasm && \
    rm -rf target

ENTRYPOINT ["/usr/bin/cat", "auto_id_domain_runtime.compact.compressed.wasm"]
